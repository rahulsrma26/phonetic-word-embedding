...
cdef class wsimdict:
    ...
    def __cinit__(self, map_file, word_file):
        self.ptr = new Dictionary(map_file.encode(), word_file.encode())
    def __dealloc__(self):
        del self.ptr
    def __len__(self):
        return self.ptr.size()
    def get_map_path(self):
        return self.ptr.getDictionaryPath().decode(self.DECODING)
    def get_map_path(self):
        return self.ptr.getMappingPath().decode(self.DECODING)
    ...
    def top_similar(self, word, k, flags=0, non_diagonal_penalty=1):
        result = self.ptr.topSimilar(word.encode(), k, flags, non_diagonal_penalty)
        return [(s, w.decode(self.DECODING)) for s, w in result]
    def __str__(self):
